...
const INITIAL_UNIT_CONVERSIONS = [
  { id: 1, itemId: 1, unitName: "CAN", multiplier: 5, baseUnitRef: "LTR", isDisplayUnit: false },
  { id: 2, itemId: 1, unitName: "JAR", multiplier: 3, baseUnitRef: "CAN", isDisplayUnit: true },
  { id: 3, itemId: 2, unitName: "PACKET", multiplier: 0.5, baseUnitRef: "LTR", isDisplayUnit: true },
  { id: 4, itemId: 3, unitName: "PACK", multiplier: 10, baseUnitRef: "PCS", isDisplayUnit: false },
  { id: 5, itemId: 3, unitName: "BOX", multiplier: 10, baseUnitRef: "PACK", isDisplayUnit: true },
];
...
function FormField({ label, children, required }) {
  return (
    <div style={{ marginBottom: 16 }}>
      <label style={{ display: "block", marginBottom: 4, fontWeight: 500, color: "#92400e" }}>
...
function ItemMasterTab({ items, setItems, setSelectedItem }) {
  ...
  const handleSave = () => {
    if (!formData.itemName || !formData.companyName || !formData.itemType || !formData.baseUnit || !formData.groupName) {
    ...
    if (editItem) {
      ...
      const newItem = {
        ...
        cstPercent: formData.cstPercent || 0,
  ...
  const columns = [
    {
      ...
      dataIndex: "itemName", width: 100
    },
    { title: <span className="text-amber-700 font-semibold">Item Type</span>, dataIndex: "itemType", width: 100 },
    { title: <span className="text-amber-700 font-semibold">Company</span>, dataIndex: "companyName", width: 100 },
    { title: <span className="text-amber-700 font-semibold">Group</span>, dataIndex: "groupName", width: 100 },
    { title: <span className="text-amber-700 font-semibold">Base Unit</span>, dataIndex: "baseUnit", align: "center", width: 100 },
    { title: <span className="text-amber-700 font-semibold">HSN / SAC</span>, render: (_, r) => r.hsnCode || r.sacCode },
    {
    ...
    { title: <span className="text-amber-700 font-semibold">Stock</span>, dataIndex: "currentStock", align: "right", width: 80 },
   {
...
>
        ...
        <div style={{ paddingTop: 16 }}>
          ...
          <Row gutter={16}>
            <Col span={12}> <FormField label="Item Name" required>
            <Input
              disabled={viewMode}
      ...
      <FormField label="SAC Code" required>
        <Select
          ...
          value={formData.sacCode}
           style={{ width: "100%" }}  
          onChange={(value) =>
            setFormData({ ...formData, sacCode: value })
          }
        >
          {SAC_CODES.map((sac) => (
          ...
          <Row gutter={16}>
            ...
            <Col span={12}>          <FormField label="Current Stock">
            <InputNumber
              disabled={viewMode}
              min={0}
              step={1}
...
function UnitConversionTab({
  ...
  const handleDelete = (id) => {
    ...
    const isReferenced = itemUnits.some(u => u.baseUnitRef === itemUnits.find(unit => unit.id === id)?.unitName);
    if (isReferenced) {
  ...
  const columns = [
    ...
    {
      title: <span className="text-amber-700 font-semibold">Conversion Chain</span>,
      render: (_, r) =>
        `1 ${r.unitName} = ${r.multiplier} ${r.baseUnitRef}`,
    },
    {
    ...
    {
      title: <span className="text-amber-700 font-semibold">Display to Customer</span>,
      align: "center",
      render: (_, r) => (
        <Button
          ...
          className={r.isDisplayUnit ? "bg-amber-600! hover:bg-amber-700! border-amber-600!" : "text-amber-700! border-amber-300! hover:text-amber-800! hover:border-amber-500!"}
        >
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          />
          <DeleteOutlined
            className="cursor-pointer! text-red-500!" onClick={() => handleDelete(record.id)}
          />
        </Space>
      ),
    },
  ...
  return (
    <>
      <Card
        title={
          <Space>
            <span style={{ color: "#92400e" }}>Unit Conversions for: {selectedItem.itemName}</span>
      ...
      >
        <div style={{ marginBottom: 16, padding: 12, background: "#fef3c7", borderRadius: 8 }}>
          ...
          <span style={{ color: "#78350f", marginLeft: 8 }}>
            You can create a unit chain! Example: 1 BOX = 10 PCS, then 1 CARTON = 10 BOX.
            The system will automatically calculate that 1 CARTON = 100 PCS.
          </span>
        </div>
      ...
      <Modal
        ...
        cancelButtonProps={{
          className: "text-amber-700! border-amber-300! hover:text-amber-800! hover:border-amber-500!"
        }}
      >
        <div style={{ paddingTop: 16 }}>
          <FormField label="Unit Name" required>
            <Input
              ...
              onChange={(e) =>
                setFormData({ ...formData, unitName: e.target.value.toUpperCase() })
              }
            />
          </FormField>
          ...
          {formData.baseUnitRef && formData.baseUnitRef !== selectedItem.baseUnit && formData.multiplier && (
            <div style={{ marginTop: 16, padding: 12, background: "#fef3c7", borderRadius: 8 }}>
              ...
              <div style={{ color: "#78350f", marginTop: 4 }}>
                1 {formData.unitName || "Unit"} = {formData.multiplier} {formData.baseUnitRef}
              </div>
            </div>
          )}
        </div>
...
function PriceManagementTab({
  ...
  const columns = [
    { title: <span className="text-amber-700 font-semibold">Unit Name</span>, dataIndex: "unitName", width: 150 },
    {
      title: <span className="text-amber-700 font-semibold">To Base Unit</span>,
      width: 200,
      render: (_, r) => {
    ...
    {
      title: <span className="text-amber-700 font-semibold">Calculated Price</span>,
      width: 200,
      render: (_, r) => {
        if (r.id === "base") {
  ...
  return (
    ...
    >
        ...
        <Row gutter={16}>
          <Col span={12}>
            <div style={{ marginBottom: 8 }}>
              <strong style={{ color: "#92400e" }}>Base Unit Price ({selectedItem.baseUnit})</strong>
            </div>
            <InputNumber
              prefix="₹"
              min={0}
      ...
      <h4 style={{ color: "#92400e", marginBottom: 16 }}>All Unit Prices (Auto-calculated)</h4>
      <Table
        className="
          [&_.ant-table-thead_th]:bg-amber-100
          [&_.ant-table-thead_th]:text-amber-800
