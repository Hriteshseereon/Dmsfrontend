...
export default function PurchaseIndent() {
  ...
  const handleReleaseContract = async (contractId) => {
    ...
    try {
      ...
      const { errorMsg, isContractExpiredOrUnapproved } =
        extractContractErrorInfo(err, "Failed to release contract");

      // Extract specific contract numbers mentioned in error (e.g. ['HA-26-0043', ...])
      ...
      const poContracts =
        selectedPoForContracts?.sale_contracts ||
        selectedPoForContracts?.sale_contract_details ||
        [];
      const matchedContract = poContracts.find(
        (c) =>
          typeof c === "object" &&
          (c.contract_id === contractId ||
            c.sale_contract_id === contractId ||
            c.id === contractId ||
            c.sale_contract_number === contractId),
      );

      const cNum =
        matchedContract?.sale_contract_number ||
        matchedContract?.contract_number ||
        matchedContract?.souda_number ||
        contractId;
      ...
      if (isContractExpiredOrUnapproved) {
        setExtendSingleModal({
          ...
          contractNumber: cNum,
          customerName:
            matchedContract?.customer_business_name ||
            matchedContract?.customer_name ||
            selectedPoForContracts?.vendor_name ||
            "",
  ...
  const handleExtendAndRelease = async () => {
    ...
    try {
      ...
      const targetId = extendSingleModal.contractId;
      setExtendSingleModal((prev) => ({ ...prev, open: false }));

      // 3. Retry release
      if (targetId) {
        setTimeout(() => {
          handleReleaseContract(targetId);
        }, 400);
      }
    } catch (err) {
