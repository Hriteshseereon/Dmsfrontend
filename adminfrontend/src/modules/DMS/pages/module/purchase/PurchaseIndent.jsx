...
import { getSalescontractGroups } from "../../../../../api/sales";
const { Option } = Select;
...
export default function PurchaseIndent() {
  ...
  const mapContractRecord = (contract, index) => ({
    key: contract.sale_contract_id || index,
    id: contract.sale_contract_id,

    saleContractNumber: contract.sale_contract_number,
    customerBusinessName: contract.customer_business_name,
    ...
    grandTotal: Number(contract.grand_total || 0),
  ...
  const fetchAvailableContracts = async () => {
    try {
      ...
      setAvailableContracts(list.map(mapContractRecord));
    } catch (err) {
      console.error(err);
      message.error("Failed to load sale contracts");
    } finally {
