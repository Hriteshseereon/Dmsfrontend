...
/* ---------------- MOCK PENDING PURCHASE CONTRACT JSON ---------------- */
const PurchaseContractRenewalJSON = [
  {
    key: 1,
    slno: 1,
...
/* ---------------- COMPONENT ---------------- */
const PurchaseContractRenewal = () => {
  const [dateRange, setDateRange] = useState(null);


  /* ---------------- FILTER LOGIC ---------------- */
const filteredData = useMemo(() => {
  return PurchaseContractRenewalJSON.filter((rec) => {
    const isRenewal = rec.approvalStatus === "RENEWAL";

    if (!dateRange) return isRenewal;

    const [start, end] = dateRange;
    const renewalDate = dayjs(rec.renewalDate);

    return isRenewal && renewalDate.isBetween(start, end, "day", "[]");
  });
}, [dateRange]);
       ...
       <Row justify="space-between" align="middle" className="mb-3">
         ...
         <Col>
           ...
           <p className="text-amber-600 text-sm">
             Month-wise purchase contract renewal records
           </p>
         </Col>
...
export default PurchaseContractRenewal;
