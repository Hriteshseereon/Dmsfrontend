...
export default function Aif() {
  ...
  const fetchData = async () => {
    try {
      const response = await getWealthEntries({ asset_category: "AIF" });
      const mappedData = response.map((item) => ({
        key: item.id,
        transactionType: item.transaction_type === "SUBSCRIPTION" ? "Investment" : (item.transaction_type.charAt(0).toUpperCase() + item.transaction_type.slice(1).toLowerCase()),
      ...
      console.error("Error fetching AIF entries:", error);
          ...
          <EyeOutlined
            ...
            onClick={async () => {
              ...
              try {
                ...
                const mappedData = {
                  transactionType: data.transaction_type === "SUBSCRIPTION" ? "Investment" : (data.transaction_type.charAt(0).toUpperCase() + data.transaction_type.slice(1).toLowerCase()),
                ...
                console.error("Error fetching AIF details:", error);
  ...
  return (
    <div>
      ...
      <div className="border border-amber-300 rounded-lg p-4 shadow-md">
        <h2 className="text-lg font-semibold text-amber-700 mb-0">AIF Investments</h2>
        <p className="text-amber-600 mb-3">Monitor alternative investment fund allocations</p>
        <Table columns={columns} dataSource={filteredData} pagination={{ pageSize: 8 }} />
      </div>

      {/* Add Modal */}
      <Modal
        title={<span className="text-amber-700 text-2xl font-semibold">Add AIF Investment</span>}
        open={isAddModalOpen}
        onCancel={() => {
        ...
        <Form
          ...
          onFinish={async (values) => {
            try {
              const payload = {
                asset_category: "AIF",
                transaction_type: values.transactionType === "Investment" ? "SUBSCRIPTION" : values.transactionType.toUpperCase(),
      ...
      {/* Edit Modal */}
      <Modal
        title={<span className="text-amber-700 text-2xl font-semibold">Edit AIF Investment</span>}
      ...
      {/* View Modal */}
      <Modal
        title={<span className="text-amber-700 text-2xl font-semibold">View AIF Investment Details</span>}
